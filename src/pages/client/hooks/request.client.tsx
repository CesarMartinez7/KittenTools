...
export default function RequestHook({
  ...
  const handleRequest = useCallback(
    async () => {
      ...
      const replaceEnvVariables = (text: string | Record<string, string>): string | Record<string, string> => {
        if (typeof text === 'string') {
            return text.replace(/{{(.*?)}}/g, (match, key) => {
                const trimmedKey = key.trim();
                const envVar = entornoActual.find(env => env.key === trimmedKey && env.enabled);
        ...
        if (typeof text === 'object') {
            ...
            for (const key in text) {
                if (typeof text[key] === 'string') {
                    newObject[key] = text[key].replace(/{{(.*?)}}/g, (match, envKey) => {
      ...
      const finalHeaders = replaceEnvVariables(cabeceras) as Record<string, string>;
