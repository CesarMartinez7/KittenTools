...
import { fetch } from '@tauri-apps/plugin-http'; // 👈 Importación corregida
...
export async function httpRequest(config) {
    if (window.__TAURI__) {
        ...
        if (formattedBody) {
            bodyPayload = fetch.Body.json(formattedBody); // 👈 Uso de 'fetch.Body.json()'
        }

        const fetchConfig = {
            method: config.method || 'GET',
            headers: config.headers,
            body: bodyPayload, // Usar el objeto Body
            query: config.params,
        };
        ...
        if (formattedConfig.data) {
            formattedConfig.data = deepReplaceEnvVariables(
                formattedConfig.data,
                useEnviromentStore.getState().entornoActual,
            );
        }
