...
// En el archivo de componentes (donde está BodyEditor)
...
const BodyEditor = memo(
  ({
    ...
    // 1. Usa `useState` para gestionar el estado local del editor
    ...
    // 2. Sincroniza el estado local cuando cambia el tab actual
    // La dependencia debe ser `currentTab` para detectar cambios en el objeto completo
    useEffect(() => {
      ...
      // Actualiza el estado local con el valor del store
      setLocalCode(newLocalCode);
    }, [currentTab]); // <-- Dependencia correcta

    // 3. Debouncing y llamada a onCodeChange (tu lógica ya es correcta)
    useEffect(() => {
      if (!currentTab) return;
    ...
    }, [localCode, onCodeChange, currentTab]);

    // 4. Manejador de cambio del editor
    ...
    return (
      <>
        <CodeEditor
          // 5. CORRECCIÓN CLAVE: Pasa el estado local `localCode` a la prop `value`
          value={localCode}
          maxHeight="85vh"
...
const ContentPanel = memo(
    ({
        ...
        // CORRECCIÓN AQUÍ: Convertir el string del editor a un objeto si es JSON
        const onCodeChange = useCallback(
            (value: string) => {
                if (!currentTab?.id) return;

                // Intenta parsear el JSON solo si el tipo de contenido es 'json'
                if (currentTab.headers['Content-Type'] === 'json') {
                    try {
                        const parsedBody = JSON.parse(value);
                        startTransition(() => {
                            updateTab(currentTab.id, { body: parsedBody });
                        });
                    } catch (e) {
                        // Si falla el parseo, actualiza con el string original
                        // para que el usuario pueda ver el error de sintaxis.
                        startTransition(() => {
                            updateTab(currentTab.id, { body: value });
                        });
                    }
                } else {
                    // Si no es JSON, guarda el valor como un string
                    startTransition(() => {
                        updateTab(currentTab.id, { body: value });
                    });
                }
            ...
            () => ({
                body: (
                    ...
                    >
                        ...
                        />
                        <pre>{JSON.stringify(currentTab.body)}</pre>
