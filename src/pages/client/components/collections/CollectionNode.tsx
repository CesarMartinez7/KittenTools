...
const CollectionItemNode = ({
  item,
  collectionId,
  level,
}: CollectionItemNodeProps) => {
  const {
    nodeData,
    collapsed,
    showBar,
  ...
  const mapperFolder = [
    {
      ...
      action: () =>
        useRequestStore
          .getState()
          .handleAddNewFolder(collectionId, item.id, 'Nueva Carpeta'),
    },
    {
      name: 'Nueva Petición',
      action: () =>
        useRequestStore
          .getState()
          .handleAddNewItem(collectionId, item.id, 'Nueva Petición'),
    },
