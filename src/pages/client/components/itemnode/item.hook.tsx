...
const useItemNodeLogic = ({ data, loadRequest, nameItem }: ItemNodeProps) => {
    ...
    const actualizarNombreEnItems = (items: Item[], oldName: string, newName: string): Item[] => {
    ...
    const eliminarItemPorNombre = (items: Item[], nameToDelete: string): Item[] => {
      return items
    ...
    const handleClick = () => {
      ...
      if (isFolder) {
        ...
        if (loadRequest) {
          loadRequest(body, language, url, method, headers, params, events, undefined);
        }
      }
    };
