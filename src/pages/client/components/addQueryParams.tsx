    ...
    <ParamsContext.Provider value={paramsFinal}>
      <div className="h-full w-full flex flex-col gap-2 my-6">
        <div className="flex gap-2">
          <button onClick={handleAddParam} className="btn gray-btn ">Añadir Parámetros</button>
          <button onClick={handleRequest} className="p-2 gray-btn">Guardar</button>
        </div>
        {params.map((param, index) => (
          <div key={index} className="flex gap-2 justify-between">
            ...
            />
            <button className="btn-black" onClick={() => handleClickDelete(index)}>
