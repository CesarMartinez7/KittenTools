...
// La función `getCaretCoordinates` es correcta y no necesita cambios.
...
const CodeEditor = ({
    ...
    }, [currentMatchIndex, findResults, value]);
    ...
    const handleKeyDown = useCallback(
        (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
            if (autocompleteSuggestions.length > 0) {
                ...
                if (e.key === 'Enter' || e.key === 'Tab') {
                    ...
                    const newCode =
                        value.substring(0, value.lastIndexOf('{{')) + `{{${suggestion}}}`;
                    onChange?.(newCode);
                    setAutocompleteSuggestions([]);
                    return;
                }
            ...
            if (e.key === 'Tab') {
                ...
                const newValue = value.substring(0, start) + '    ' + value.substring(end); // Usé 4 espacios para el tab.
                onChange?.(newValue);
        ...
        [autocompleteSuggestions, activeSuggestionIndex, value, onChange],
    );
    ...
    // Eliminar el useEffect que maneja el estado `isBodyNull`
                ...
                <div className="relative flex justify-between items-center text-[8px] text-gray-500 dark:text-zinc-400 bg-gray-200/70 dark:bg-zinc-950/50 border-t border-gray-300 dark:border-zinc-800 px-2 py-1.5 shadow-sm">
                    <div className="flex items-center gap-1">
                        <button className="button-code-tools" onClick={HandlersIdentarBody}>
                            ...
                            <span className="hidden sm:inline font-black">Beautify</span>
                        </button>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="hidden sm:inline">
                            {language.toUpperCase()} | {value.length} caracteres | {lineCount}{' '}
                            líneas
                        </span>
                    </div>
                </div>
