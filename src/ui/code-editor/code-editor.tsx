...
const CodeEditor = ({
	...
	const handleCLickReplaceTextFirst = () => {
		...
		if (!code.includes(from)) return toast.error('El valor a buscar no se encuentra en el texto');
		...
		<main className="border rounded-xl overflow-hidden border-zinc-800 relative">
			<AnimatePresence mode="wait">
				{isOpenBar && isEditable && (
					...
					>
						<input ref={inputRefTextOld} type="text" autoFocus className="input-base" placeholder="Valor a buscar" />
						<input ref={inputRefTextNew} type="text" className="input-base" placeholder="Valor a reemplazar" />
						<div className="flex h-6 gap-2">
							<button className="bg-gradient-to-r flex-1 from-green-400 to-green-500 p-1 rounded-md text-xs"
								onClick={handleCLickReplaceTextFirst}>
								Reemplazar primero
							</button>
							<button className="bg-gradient-to-r flex-1 from-sky-400 to-sky-900 p-1 rounded-md text-xs"
			...
			<div className={`relative flex text-xs overflow-hidden bg-zinc-900/80 backdrop-blur-3xl ${classNameContainer}`}>
				{/* Line Numbers */}
				<div
					ref={lineNumbersRef}
					...
					<span className="text-green-400">
						{(() => {
							try {
								...
								return <Icon icon="tabler:x" width={13} height={13} color="red" />;
							}
						})()}
					</span>

					<span className="hidden sm:inline">
						{language.toUpperCase()} | {code.length} caracteres | {lineCount} líneas
					</span>
				</div>
			</div>
		</main>
